...
class DIV2K_DW(data.Dataset):
    def __init__(self, args, train=True):
        ...
        split = 'train'
        dir_HR = 'DIV2K_{}_LR_mild_register'.format(split)
       # dir_LR = 'DIV2K_{}_LR_bicubic'.format(split)
        dir_LR = 'DIV2K_{}_LR_mild'.format(split)
        ...
        if self.args.ext != 'pack':
            ...
            if self.fullTrain:
                ...
                if False:    
                    ...
                    packname = 'pack.pt' if self.train else 'packv.pt'
                    name_tar = os.path.join(self.apath, dir_HR,str(xs), packname)
                    print('\tLoading ' + name_tar)
                    self.pack_in = []
                    self.pack_tar = torch.load(name_tar)
                    if self.train:
                        self._save_partition(
                                self.pack_tar,
                                os.path.join(self.apath, dir_HR,xs, 'packv.pt'))                    
                                        
                #for i, xs in enumerate(x_NofullTrain_Input):
                    
                xs = 'X4_2' #x_NofullTrain_Input[0]
...
#            print(img_in.shape)
#            print(img_tar.shape)
#            img_tar = img_tar[100:ih * scale-100, 100:iw * scale-100, :]
#            img_in = img_in[(ih//2)-50:(ih//2)+50,(iw//2)-50,(iw//2)+50,:]
#            img_tar = img_tar[(ih//2)*2-100: (ih//2)*2+100,(iw//2)*2-100:(iw//2)*2+100,:]
