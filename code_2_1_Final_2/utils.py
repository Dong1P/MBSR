...
class checkpoint():
    ...
    def save_results(self, idx, input, output, target, scale):
        if self.args.save_results:
            ...
            dir = '/home/kky/Dongwon/SuperResolution/Data/NTIRE/DIV2K/DIV2K_train_LR_mild/test_results'
            filename = '%s/%04dx%d'%(dir, idx + 1, scale)
            #for v, n in (output, 'SR'):#, (target, 'GT'):
            v = output
            n = 'SR_1'
            tu.save_image(
...
def x8_forward(img, model, precision='single'):
    ...
    outputlist = [chop_forward(aug,model,2) for aug in inputlist]
...
#    (_, ic, ih, iw) = input.size()
#    input = quantize(input[:,:,100:ih-100,100:iw-100].data[0], rgb_range)
    (_, c, h, w) = input.size()
