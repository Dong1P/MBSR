...
class DIV2K_DW(data.Dataset):
    def __init__(self, args, train=True):
       ...
       # dir_LR = 'DIV2K_{}_LR_bicubic'.format(split)
        ...
        dir_mild = 'DIV2K_{}_LR_mild'.format(split)
        ...
        if self.args.ext != 'pack':
            ...
            if self.fullTrain:
                ...
                xs = 'X4_4' #x_NofullTrain_Input[0]
                name_in = os.path.join(self.apath, dir_mild,str(xs), packname)
                print('\tLoading ' + name_in)
                self.pack_in.append(torch.load(name_in))
                if self.train:
                    self._save_partition(
                            self.pack_in[0],
                            os.path.join(self.apath, dir_mild, xs, 'packv.pt'))
