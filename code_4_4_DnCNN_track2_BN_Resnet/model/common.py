...
def default_conv(in_channelss, out_channels, kernel_size, bias=True):
    ...
    init.xavier_uniform(net.weight,gain = np.sqrt(2))
    init.constant(net.bias,0.1)
    return net
    ...
    def __init__(self, conv, scale, n_feat, bn=False, act=False, bias=True):
        ...
        if (scale & (scale - 1)) == 0:    # Is scale = 2^n?
            for _ in range(int(math.log(scale, 2))):
                modules.append(_PyramidPoolingModule(n_feat,n_feat,(1,2,3)))
                #modules.append(conv(n_feat, 4 * n_feat, 3, bias))
                modules.append(conv(4*n_feat,4*n_feat,3,bias))
    ...
    def __init__(self, in_dim, reduction_dim, setting):
        ...
        for s in setting:
            self.features.append(nn.Sequential(
                ...
                #nn.Conv2d(in_dim, reduction_dim, kernel_size=3,padding=(3 // 2), bias=True),
