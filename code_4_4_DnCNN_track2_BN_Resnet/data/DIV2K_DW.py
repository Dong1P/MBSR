...
class DIV2K_DW(data.Dataset):
    def __init__(self, args, train=True):
        ...
        self.fullInputScale = '4'#args.fullInputScale
        split = 'train'
        dir_HR = 'DIV2K_{}_HR'.format(split)
        dir_LR = 'DIV2K_{}_LR_mild_register'.format(split)
        dir_LR_mile = 'DIV2K_{}_LR_mild'.format(split)
        x_NofullTrain_Input = ['X{}'.format(s) for s in '4']
        ...
        if self.args.ext != 'pack':
            ...
            if self.fullTrain:
                ...
                if args.fullTargetScale == '1':    
                    ...
                    packname = 'pack.pt' if self.train else 'packv.pt'
                    name_tar = os.path.join(self.apath, dir_LR,str(xs), packname)
                    print('\tLoading ' + name_tar)
                    self.pack_in = []
                    self.pack_tar = torch.load(name_tar)
#                    self.train = True
                    if self.train:
                        self._save_partition(
                                self.pack_tar,
                                os.path.join(self.apath, dir_LR,str(xs), 'packv.pt'))                    
                                        
                #for i, xs in enumerate(x_NofullTrain_Input):
                    
                xs = x_NofullTrain_Input[0]
                name_in = os.path.join(self.apath, dir_LR_mile,str(xs), packname)
                print('\tLoading ' + name_in)
                self.pack_in.append(torch.load(name_in))
                if self.train:
                    self._save_partition(
                            self.pack_in[0],
                            os.path.join(self.apath, dir_LR_mile, str(xs), 'packv.pt'))
